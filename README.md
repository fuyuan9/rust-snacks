...
- **連載の一括事前生成**: 連載記事と判定された場合、初回生成時に Part 2〜N まで全パートを一括でLLMで生成し、`needs_review` 状態でD1にストック。次回Cron実行時に順次自動公開。
- **リポジトリの賢い選定**: スター・フォーク・更新頻度に基づくスコアリングで最良のリポジトリを自動選定。同一リポジトリは **90日のクールダウン期間** を設けて重複を避ける。
- **リポジトリ除外フィルター**: 公式コアリポジトリ（`rust-lang/rust`）および100MB超の巨大リポジトリは収集・選定の対象から自動除外。
...
1. **candidate_collection**: GitHub Search API でスター数100以上・アーカイブなし・フォークなし等の条件を満たすRustリポジトリ候補を最大30件収集し、D1に一時保存。
   - **除外フィルター**: `rust-lang/rust` (公式コアリポジトリ) および サイズが **100MB超 (100,000 KB超)** のリポジトリは収集時点で自動スキップ。
2. **repository_selection**: 未完の連載（`series_index < series_total`）があるリポジトリを最優先で選定。無い場合は、収集候補からスコアリング（スター・フォーク・更新頻度・説明文の有無）を元に自動選定。
   - 同一リポジトリは直近 **90日以内** に記事化済みの場合はスキップ（クールダウン）。
   - `rust-lang/rust` は選定フェーズでも二重に安全除外される。
3. **snapshot**: 選定リポジトリのREADME、Cargo.toml、主要コード（`lib.rs`/`main.rs`）を取得し、制限サイズ内に切り詰めてR2およびD1にスナップショットとして保存。
4. **article_generation**: スナップショットをLLM（デフォルト: `gemini-3.5-flash`）に入力し、解析と記事執筆を行います。
   - **品質ゲート (Quality Gate)** を自動検証（以下の基準を満たす必要があります）:
     - 本文が **15,000文字以内**
     - Tipsセクションが **5個以内**
     - Mermaidダイアグラムが **3個以内**
     - コードブロックが **50行以内**
...
各記事は以下のセクション構成で統一されています：

```
## 1. 概要
## 2. アーキテクチャ（Mermaidダイアグラム付き、最大3図）
## 3. この記事で学べること（最大5項目）
## 4. 実践的な実装・コード解説
## 5. 実務に持ち帰れるTips（最大5個）
## 6. トレードオフと注意点
